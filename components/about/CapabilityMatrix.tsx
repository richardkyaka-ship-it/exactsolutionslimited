      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-gray-900 border border-gray-900">
          {CAPABILITIES.map((item, index) => (
            <div key={index} className="bg-black p-12 group hover:bg-dark-light transition-colors duration-500">
              <item.icon className="w-8 h-8 text-primary mb-8 group-hover:scale-110 transition-transform duration-500" />
              <h3 className="text-[10px] text-gray-500 uppercase tracking-[0.3em] font-medium mb-4">
                0{index + 1} // {item.title}
              </h3>
              <div className="h-px w-12 bg-primary/30 mb-6 group-hover:w-24 transition-all duration-500" />
              <p className="text-2xl font-light text-white group-hover:text-primary transition-colors duration-300">
                {item.spec}
              </p>
            </div>
          ))}
