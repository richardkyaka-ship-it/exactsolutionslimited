...
export default function TimelineSection() {
  return (
    <section className="px-4 md:px-12 lg:px-20 py-20 md:py-32 bg-light-lighter dark:bg-dark-light border-b border-light-border dark:border-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative px-2 sm:px-0">
          ...
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-light-border dark:bg-gray-800 -translate-x-1/2" />
              ...
              >
                ...
                <div className={`w-full md:w-1/2 pl-12 md:pl-0 ${index % 2 === 0 ? 'md:pl-16' : 'md:pr-16'}`}>
                  <div className={`flex flex-col ${index % 2 === 0 ? 'md:items-start' : 'md:items-end'}`}>
                    ...
                    <h3 className={`text-base md:text-lg font-light text-light-text dark:text-white mb-2 ${index % 2 === 0 ? 'text-left' : 'md:text-right'}`}>
                      {item.title}
                    </h3>
                    <div className={`flex items-center gap-2 ${index % 2 === 0 ? 'flex-row' : 'md:flex-row-reverse'}`}>
                      <div className="h-px w-4 bg-light-border dark:bg-gray-700" />
                      <span className="text-[9px] md:text-[10px] text-light-text-muted dark:text-gray-500 uppercase tracking-[0.2em]">{item.metric}</span>
                    </div>
                  </div>
                </div>
              </motion.div>
