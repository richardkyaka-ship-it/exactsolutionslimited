...
export default function ProductsHighlight() {
  ...
  return (
    <section className="px-4 md:px-12 lg:px-20 py-20 md:py-32 bg-black">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 md:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8">
          <div>
            ...
            <p className="text-gray-500 uppercase tracking-widest text-[10px] md:text-sm">Browse industrial equipment</p>
          </div>
          <FastLink
            href="/products"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {loading ? (
            <div className="col-span-full py-12 text-center text-gray-800">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              className="group bg-dark-light border border-gray-900 hover:border-primary/50 transition-all duration-500 flex flex-col"
            >
              <div className="relative aspect-square overflow-hidden bg-dark-lighter">
                {product.images && product.images.length > 0 && product.images[0] ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-dark-lighter">
                    <span className="text-[9px] text-gray-700 uppercase tracking-widest font-mono">No Image</span>
                  </div>
                )}
              ...
              <div className="p-5 md:p-6 space-y-3 md:space-y-4">
                <h3 className="text-base font-light text-white uppercase tracking-tight line-clamp-1 group-hover:text-primary transition-colors">
                  {product.name}
                </h3>
                <p className="text-[9px] md:text-[10px] text-gray-500 font-mono uppercase tracking-widest border-l border-gray-800 pl-3">
                  {product.keySpecs[0]}
                </p>
                <FastLink
                  href={`/products/${product.id}`}
                  className="inline-block w-full py-2.5 md:py-3 border border-gray-800 text-gray-400 text-[10px] uppercase tracking-[0.2em] font-medium text-center hover:border-white hover:text-white transition-all duration-300"
                >
                  View Details
                </FastLink>
              </div>
