...
export default function NavigationMatrix() {
  ...
  return (
    <section className="bg-light dark:bg-dark border-y border-light-border dark:border-dark-border">
        ...
        {PANELS.map((panel, index) => {
          ...
          return (
            <Link 
              ...
              className={`relative group h-[320px] sm:h-[400px] md:h-[600px] border-light-border dark:border-dark-border transition-all duration-500 overflow-hidden flex flex-col justify-end p-6 sm:p-8 md:p-12 ${
                index !== PANELS.length - 1 ? 'border-b lg:border-b-0 lg:border-r' : ''
              }`}
            >
              {/* Background Pattern Placeholder */}
              <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.03] group-hover:opacity-[0.07] dark:group-hover:opacity-[0.07] transition-opacity duration-700 pointer-events-none">
                <svg width="100%" height="100%" className="w-full h-full text-light-text-subtle dark:text-white">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div>
                  <h3 className="text-2xl md:text-3xl font-light text-light-text dark:text-dark-text-primary uppercase tracking-tight group-hover:text-primary transition-colors">
                    {panel.title}
                  </h3>
                  <div className="h-px w-8 bg-primary/40 my-4 group-hover:w-16 transition-all duration-500" />
                  <p className="text-xs font-mono text-light-text-muted dark:text-gray-500 uppercase tracking-widest">
                    {panel.metric}
                  </p>
                </div>

                <div className="text-[10px] text-light-text dark:text-dark-text-primary uppercase tracking-[0.3em] font-medium flex items-center gap-2 group-hover:translate-x-2 transition-transform duration-300">
