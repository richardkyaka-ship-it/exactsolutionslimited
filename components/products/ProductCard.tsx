  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <h3 
          className="text-lg font-light text-white uppercase tracking-tight mb-6 group-hover:text-primary transition-colors cursor-pointer line-clamp-2 min-h-[3.5rem]"
          onClick={() => onOpenSpecs(product)}
        >
          {product.name}
        </h3>
        ...
        <ul className="space-y-2 mb-8">
          {product.keySpecs.map((spec, i) => (
            <li key={i} className="flex items-center gap-2 text-[11px] text-gray-400 font-mono">
        ...
        <div className="mt-auto grid grid-cols-2 gap-3">
          ...
          <button
            onClick={() => onOpenSpecs(product)}
            className="px-4 py-2 border border-gray-800 text-gray-400 text-[10px] uppercase tracking-[0.2em] font-medium hover:border-white hover:text-white transition-all duration-300"
          >
            Specs
          </button>
        </div>
